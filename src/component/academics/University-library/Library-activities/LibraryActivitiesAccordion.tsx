    ...
    <div className="tl-8-accordion">
        {array.map((item) => (
          <div className={`tl-8-accordion-item ${openAccordion === item.id ? 'open' : ''}`} key={item.id}>
            ...
            >
              <h6 className="tl-8-accordion-item-title text-[#4f70b6]">
                <strong>{item.title}</strong>
              </h6>
              <span className="tl-8-accordion-item-expand-icon"></span>
            </div>

            <p className={`tl-8-accordion-item-body ${openAccordion === item.id ? 'open' : ''}`} dangerouslySetInnerHTML={{ __html: item.desc ? item.desc.replace(/\n/g, '<br/>') : '' }} />
          </div>
        ))}
      </div>
  )
