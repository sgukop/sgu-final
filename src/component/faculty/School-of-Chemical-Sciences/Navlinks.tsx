const NavLinks = [
    { title: "About Department",path: "/chemical-dept-about"},
    { title: "Programs", path: "/chemical-dept-programmes" },
    { title: "Faculty", path: "/chemical-dept-faculty" },
    { title: "Academic Infrastructure", path: "/chemical-dept-infrastructure" },
    { title: "BOS", path: "/chemical-dept-BOS" },
    { title: "Research & Publications", path: "/chemical-dept-research" },
    { title: "Professional Bodies", path: "/chemical-professional-bodies" },
    { title: "Activities", path: "/chemical-dept-newactivities" },
    { title: "Curriculum Structure and Syllabus", path: "/che-dept-syllabus" },
   /*{ title: "Placement", path: "/chemical-dept-placements" }, */
    { title: "MOU", path: "/chemical-dept-MOU" },
     { title: "Alumni", path: "/chemical-dept-alumni" },
    { title: "Conference / Guest lecture / Invited talk / Workshop / FDP attended", path: "/chemical-dept-conference" },
    { title: "Achievements", path: "/chemical-dept-achievements" },
    { title: "Associations", path: "/chemical-dept-associations" },
    { title: "Internship & Placements", path: "/chemical-dept-internship" },
   ];
  
  export default NavLinks;